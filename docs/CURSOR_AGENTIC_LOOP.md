...
## Execution policy (serial writer, parallel read-only)

For multi-phase execution in `src/ira/pipeline_loop.py`, use this policy:

- **One writer at a time:** only one mutating phase may run at once. If a phase is already `RUNNING`, another mutating phase must wait (or abort and retry later).
- **Serial by default:** phase agent execution is serial unless the phase is explicitly marked read-only.
- **Parallel read-only only:** fan-out is allowed only when `delegation_type` is `readonly` or `read_only`. Use this for research/verification style sub-work where no state mutation is expected.
- **Do not parallelize mutating phases:** any phase that can change code, CRM, memory, files, or other durable state stays serialized to prevent conflicts and drift.

This keeps long-running loops stable while still giving throughput gains on safe read-only work.

---

## MCP agent loop vs `ira task`

| Path | Use when |
|:-----|:---------|
| **`plan_task` → `execute_phase` → `generate_report`** (MCP) | Multi-phase work that needs **validation contracts**, per-phase assertions, replan/clarify, or **standing objectives** |
| **`ira task`** | Same semantics as MCP for CLI/scripts; workspace at `data/tasks/<task_id>/` (`PLAN.md`, `MEMORIES.md`, `CONTRACT.json`, `phases/`) |
| **`ira ask`** | Single-shot questions; full 17-step pipeline |

Prefer the agent loop when work must not “lose the plot” across phases. See [LONG_RUNNING_HARNESS.md](LONG_RUNNING_HARNESS.md) and [AGENT_LOOP_STANDARD.md](AGENT_LOOP_STANDARD.md) (full harness in the private operator repo; public stubs ship in ira-universe).
